	...
	$$renderer.component(($$renderer) => {
		...
		if (data.gallery.length === 0) {
			...
			for (let i = 0, $$length = each_array.length; i < $$length; i++) {
				...
				if (brokenImageIds.includes(item.id)) {
					...
					$$renderer.push("<!--[-1-->");
					$$renderer.push(`<img class="thumb svelte-naetws"${attr("src", item.imageUrl)}${attr("alt", item.alt)} loading="lazy" decoding="async" onerror="this.__e=event"/>`);
				}
		...
		if (activeItem()) {
			...
			if (brokenImageIds.includes(activeItem().id)) {
				...
				$$renderer.push("<!--[-1-->");
				$$renderer.push(`<img class="lightbox-img svelte-naetws"${attr("src", activeItem().imageUrl)}${attr("alt", activeItem().alt)} loading="eager" decoding="async" onerror="this.__e=event"/>`);
			}
...
//# sourceMappingURL=_page.svelte-C4Uus12P.js.map
